...
# Step 1: Create the simple_push environment with render_mode for visualization
env = simple_push_v3.env(max_cycles=25, render_mode='human')  # Set render_mode='human' for visualization
...
# Pad observations to ensure all agents have consistent observation sizes
...
model.save("mappo_simple_push")

# Step 7: Testing the trained model
# Reload the model (optional)
model = PPO.load("mappo_simple_push")

# Recreate the environment for testing with render_mode='human'
test_env = simple_push_v3.env(max_cycles=25, render_mode='human')
