...
# Step 1: Create the simple_adversary environment with render_mode for visualization
env = simple_adversary_v3.env(max_cycles=25, render_mode='human')  # Set render_mode='human' for visualization
...
# Pad observations and actions to ensure consistency between agents
...
model.save("mappo_simple_adversary")

# Step 7: Testing the trained model
# Reload the model (optional)
model = PPO.load("mappo_simple_adversary")

# Recreate the environment for testing with render_mode='human'
test_env = simple_adversary_v3.env(max_cycles=25, render_mode='human')
...
parallel_test_env = ss.pad_action_space_v0(parallel_test_env)
