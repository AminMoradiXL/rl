...
# Step 1: Create the simple_spread environment with render_mode for testing
env = simple_spread_v3.env(N=3, max_cycles=25, render_mode='human')  # Set render_mode here

# Step 2: Convert the AEC environment to a parallel environment
parallel_env = aec_to_parallel(env)

# Step 3: Wrap the parallel environment with SuperSuit for compatibility
...
model.save("multiagent_ppo_simple_spread")

# Step 7: Testing the trained model
# Reload the model (optional)
model = PPO.load("multiagent_ppo_simple_spread")

# Recreate the environment for testing with render_mode='human'
test_env = simple_spread_v3.env(N=3, max_cycles=25, render_mode='human')
