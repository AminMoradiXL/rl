...
action_space_size = 5  # Discretize the action space into 5 possible actions
...
def discretize_action(action_idx):
    """Convert a discrete action index into a continuous action."""
    # Define the range of actions (torque) for Pendulum-v1 between -2.0 and 2.0
    action_range = np.linspace(-2.0, 2.0, action_space_size)
...
plt.title('Total Reward per Episode over Time (Pendulum-v1)')
plt.show()
